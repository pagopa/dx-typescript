...
import * as NonceModel from "../models/nonce";
  ...
  pipe(
    ...
    TE.chainFirst(nonce =>
      pipe(
        ...
        TE.chain(flow(NonceModel.create(nonce), TE.mapLeft(errorToHttpError)))
      )
    )
  );
...
export const GenerateNonceFn = httpAzureFunction(makeGenerateNonce);
